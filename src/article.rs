...
impl ArticleModule {
    ...
    fn new_article(req: &mut Request) -> Result<Response> {
        ...
        if let Ok(article) = ret {
            Ok(Response::new_check(vec![article]))
        } else {
            ...
            let mut res = Response::new_uncheck(StatusCode::BAD_REQUEST, json_result);
            res.set_header(
    ...
    fn version(_req: &mut Request) -> Result<Response> {
        ...
        let response = Response::from_str_uncheck(StatusCode::OK, ret);
